...
fact = [
	'The scientific term for brain freeze is "sphenopalatine ganglioneuralgia."The scientific term for brain freeze is "sphenopalatine ganglioneuralgia."',
	...
	"Daniel Radcliffe was allergic to his Harry Potter glasses. Harry Potter wearing his glasses. He had an allergy to nickel, and they were quickly replaced with hypoallergenic specs. Also, did you know that his glasses had no lenses? This was to stop the reflection from anything happening behind the scenes. The glass lens was added in post-production.",
	...
	"Apple paid a couple $1.7 million dollars for their plot of land, which was only worth $181,700. While Apple was building a huge data center in the middle of North Carolina, they wanted to occupy the area of a couple that had lived there for 34 years. When the couple refused to leave, Apple paid them $1.7 million dollars for their land.",
	...
	"In 2007, Scotland spent 125,000 Euros devising a new national slogan. The winning entry was: 'Welcome to Scotland.'",
...
factselector = document.getElementById("fact");
count = 0;
placeholder = factselector.innerText = fact[count];
cl = factselector.classList;

timeFunction = new Date();
time = timeFunction.toLocaleTimeString();
date = timeFunction.toDateString();

b = time.split(":"); // spliting time by :
c = Array(time.split(":")); // convert to array
period = String(b.pop()).split(" ").pop(); // getting either am or pm
hour = c[0][0]; // hour
min = c[0][1]; // minutes
time = `${hour}:${min} ${period}`; // results

dateTargeter = document.getElementById("date");
timeTargeter = document.getElementById("time");
dateTargeter.innerText = date;
timeTargeter.innerText = time;

function next() {
	count++;
	factselector.innerText = fact[count];
	console.log(count);
	if (count > 99) {
		count = 99;
		factselector.innerText = fact[99];
	}
